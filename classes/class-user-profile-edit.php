...
if( ! class_exists( 'UserProfileEdit' ) ) {
    ...
    class UserProfileEdit {
        ...
        public function __construct( $args ){
            ...
            add_action( 'personal_options_update', array( $this, 'save_update_user_profile' ) );
            add_action( 'edit_user_profile_update', array( $this, 'save_update_user_profile' ) );
        ...
        public function save_update_user_profile($user_id){
            ...
            foreach ($this->get_panels() as $panelsIndex=>$panel):
                foreach ($panel['sections'] as $sectionIndex=>$section):
                    foreach ($section['options'] as $option):
                        ...
                        $option_save[] = $option_value;

                        update_user_meta($user_id, $option['id'], $option_value);
            ...
            update_option('option_save_xyz', $option_save);
                ...
                foreach ($this->get_panels() as $panelsIndex=>$panel):
                    foreach ($panel['sections'] as $sectionIndex=>$section):
                        ...
                        foreach ($section['options'] as $option):
                            ?>
                            <tr>
                                <th><label for="<?php echo $option['id']; ?>"><?php echo $option['title']; ?></label></th>
                                <td>
                                    <?php

                                    $this->field_generator($option, $user_id)
                                    ?>
                                </td>
                            </tr>
                        <?php
        ...
        public function field_generator( $option, $user_id ) {
            ...
            $option_value 	 		= get_user_meta($user_id,  $id, true );
